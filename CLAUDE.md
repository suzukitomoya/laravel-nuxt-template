...
- 日本語で記述してください。
- **重要** すべてのレビューコメントは必ずインラインコメントで記載してください。インラインコメントは`mcp__github_inline_comment__create_inline_comment`を使用して作成できます。
  - 出力処理の最後に必ず確認：インラインコメントが作成されていない指摘項目はありませんか？作成漏れを洗い出し、インラインコメントを漏れなく作成してください。
